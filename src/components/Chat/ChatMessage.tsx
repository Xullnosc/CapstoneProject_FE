    ...
    <div className={`flex items-end gap-3 w-full animate-fade-in mb-1 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      {!isMe && (
        <div 
          ...
          className={`flex-shrink-0 mb-1 w-9 h-9 cursor-pointer transition-transform hover:scale-110 active:scale-95 ${!showAvatar ? 'opacity-0 select-none pointer-events-none' : 'opacity-100'}`}
        >

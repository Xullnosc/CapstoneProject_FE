...
const HodDecisionModal: React.FC<HodDecisionModalProps> = ({
  ...
  const handleAiReview = async () => {
    ...
    try {
      ...
      const aiReviewedCount = duplication.matches.filter((m) => m.aiSkipped === false).length;
      notices.push(
      ...
      notices.push(
        duplication.aiEnriched
          ? `AI semantic validation completed for ${aiReviewedCount} match(es).`
          : "AI semantic validation was skipped (disabled or no medium/high candidates)."
      );

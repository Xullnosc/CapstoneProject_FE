...
const Header = () => {
    ...
    const getLinkClass = (path: string) => {
        return isActive(path)
            ? "flex items-center gap-2 text-orange-600 font-semibold px-3 py-2 rounded-xl hover:bg-orange-50 transition-all duration-200"
            : "flex items-center gap-2 text-gray-500 font-medium hover:text-orange-600 px-3 py-2 rounded-xl hover:bg-orange-50 transition-all duration-200";
    };

    const getMobileLinkClass = (path: string) => {
        return isActive(path)
            ? "flex items-center gap-3 text-orange-600 font-medium px-4 py-3 rounded-xl bg-orange-50 transition-all duration-200"
            : "flex items-center gap-3 text-gray-700 font-medium px-4 py-3 rounded-xl hover:bg-orange-50 hover:text-orange-600 transition-all duration-200";
    };
        ...
        <header className="h-16 bg-white shadow-sm border-b border-gray-100 flex items-center justify-between px-4 sm:px-6 lg:px-8 relative z-50">
            ...
            <Sidebar visible={visible} onHide={() => setVisible(false)} className="w-[80vw] sm:w-[20rem]">
                <div className="flex flex-col h-full">
                    ...
                    <div className="flex flex-col gap-2">
                        <a href="/home" className={getMobileLinkClass('/home')}>
                            <i className="pi pi-home text-xl"></i>
                            <span>Homepage</span>
                        </a>
                        <div onClick={() => navigate('/teams/team')} className={`${getMobileLinkClass('/teams')} cursor-pointer`}>
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden sm:flex items-center gap-8 lg:gap-16">
                ...
                <nav className="flex items-center gap-4 lg:gap-10">
                    ...
                    <div onClick={() => navigate('/teams/team')} className={`${getLinkClass('/teams')} cursor-pointer`}>
                        <i className="pi pi-users text-xl"></i>

...
const Login = () => {
  ...
  const saveAuthAndRedirect = (response: { token: string; userInfo: { fullName: string; roleName: string } }) => {
    ...
    <div className={styles.loginContainer}>
      ...
      <div className={styles.loginCard}>
        <div className="text-center mb-6">
          <img src={fptLogo} alt="App Logo" className="mx-auto h-14 object-contain" />
        </div>
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-lg font-medium text-sm whitespace-nowrap transition ${tab === 'google' ? 'bg-[#F26F21] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
          >
            Đăng nhập Google
          </button>
          <button
          ...
          <>
            ...
            <div className="relative my-5">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-white text-gray-500">Login with your powered email</span>
              </div>
            </div>
            <button
              onClick={handleGoogleLogin}
              disabled={isLoading}
              className={`${styles.googleButton} ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              {isLoading ? (
